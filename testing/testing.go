...
const version = "1.0.0" // Update with your desired version number
...
func (c *CustomCollector) updateMetrics(r *http.Request, domain, port, uri, target string) error {
	...
	url := fmt.Sprintf("http://%s:%s@%s:%s%s", username, password, domain, port, uri)
...
func main() {
	...
	http.HandleFunc("/metrics", func(w http.ResponseWriter, r *http.Request) {
		...
		go func() {
			defer wg.Done()
			if err := customCollector.updateMetrics(r, domain, port, uri, target); err != nil {
				once.Do(func() {
					scrapeError = err
				})
			}
